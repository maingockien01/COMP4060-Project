...
func (k Keeper) OnRecvRequestIndexPacket(ctx sdk.Context, packet channeltypes.Packet, data types.RequestIndexPacketData) (packetAck types.RequestIndexPacketAck, err error) {
	...
	resource, found := k.oracleKeeper.GetResource(ctx, data.GetIndex())

	if !found {
		return packetAck, sdkerrors.Wrap(types.ErrNotFoundResource, "Resource is not found")

	}

	packetAck.Index = resource.Index
	packetAck.LastUpdatedAt = resource.LastUpdatedAt
	packetAck.Path = resource.Path
	packetAck.Value = resource.Value
